 ...
 * */
export type NumberLike = jsbn.BigInteger | number | string;
 ...
 * @returns {BigInteger}
 */
function convertToBN(input: NumberLike): jsbn.BigInteger {
...
export class PrivateKey {
    ...
    private lambda: jsbn.BigInteger;
    private mu: jsbn.BigInteger;

    constructor(lambda, mu, public public_key) {
...
export class PublicKey {
    ...
    private get_random_lt_n(){
        var r: jsbn.BigInteger;

        do {
            //r = new bn(1 + Math.log(this.n)/Math.LN2, 1, rng);
            r = new bn(
                bn.ONE.add(bigIntegerLog(this.n).divide(new bn(Math.LN2.toString()))),
                1, rng);
            // make sure r <= n
     ...
     */
    public raw_encrypt(plaintext_in: NumberLike, r_value_in: NumberLike) {
        // if plaintext isn't a bignum convert it...
        var plaintext: jsbn.BigInteger = convertToBN(plaintext_in);
        var r_value: jsbn.BigInteger = convertToBN(r_value_in);

        var nude_ciphertext: jsbn.BigInteger;
        if( (this.n.subtract(this.max_int).compareTo(plaintext) <= 0) && (plaintext < this.n)){
            ...
            nude_ciphertext = this.g.modPow(<jsbn.BigInteger>plaintext, this.nsquare);
        }
     ...
     */
    public encrypt(value, precision, r_value) {
 ...
 */
export function generate_paillier_keypair(n_length: number): KeyPair {
    var keysize;
...
export class EncodedNumber {
    ...
    static FLOAT_MANTISSA_BITS = 53;

    /** Compute the logarithm of x with given base */
    static log(x: number, base: number) {
        return Math.log(x)/Math.log(base);
    };

    static LOG2_BASE = EncodedNumber.log(16, 2);

    // Save a reference to the base as a BigInteger
    static BASE_BN = new bn(EncodedNumber.BASE.toString(), 10);

    private static frexp_exponent(value) {
     ...
     */
    constructor(public public_key: PublicKey, encoding, public exponent: number) {
     ...
     */
    public decrease_exponent_to(new_exp) {
     ...
     * @returns {EncodedNumber}
     */
    static encode(public_key, scalar, precision, max_exponent) {
        ...
        if(typeof precision === "undefined"){
            ...
            if(isFloat(scalar)){
                ...
                let bin_lsb_exponent = bin_flt_exponent - EncodedNumber.FLOAT_MANTISSA_BITS;
 ...
 */
export class EncryptedNumber{

    constructor(public_key, ciphertext, exponent) {
